	...
	class portfolio_expert_About
	{
		...
		public function check_if_plugin_active($slug)
		{
			...
			if (file_exists($path)) {
				...
				$plugin_path = $slug . '/' . $plugin_root_file . '.php';
				$needs = in_array( $plugin_path, $active_plugins, true ) ? 'deactivate' : 'activate';

				return array('status' => in_array( $plugin_path, $active_plugins, true ), 'needs' => $needs);
			}
...
?>
		...
		public function recommended_actions()
		{
			...
			?>
							<p class="plugin-card-<?php echo esc_attr($action_value['plugin_slug']) ?> action_button <?php echo esc_attr( ($active['needs'] !== 'install' && $active['status']) ? 'active' : '' ); ?>">
