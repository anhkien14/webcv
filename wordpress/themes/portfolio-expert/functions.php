 ...
 */
function portfolio_expert_resource_hints( $urls, $relation_type ) {
...
add_filter( 'wp_resource_hints', 'portfolio_expert_resource_hints', 10, 2 );
...
/**
 * Fallback function for main navigation with max 7 items including Home
 */
function portfolio_expert_fallback_menu($args = array())
{
	...
	// Get pages for fallback menu (limited to 6 pages + Home = 7 total)
	$pages = get_pages(array(
		'sort_column' => 'menu_order, post_title',
		'number' => 6, // Limit to 6 pages since we'll add Home
		'meta_key' => '',
		'meta_value' => '',
		'hierarchical' => 1,
		'exclude' => '',
	...
	if (!empty($args['menu_id'])) {
		$output .= '<ul id="' . esc_attr($args['menu_id']) . '"';
	} else {
		$output .= '<ul';
	}
	...
	$output .= '<li class="menu-item menu-item-home' . $home_class . '"><a href="' . esc_url(home_url('/')) . '">' . esc_html__('Home', 'portfolio-expert') . '</a></li>';

	// Add pages (maximum 6 to keep total at 7 with Home)
	$count = 0;
	foreach ($pages as $page) {
		if ($count >= 6) break; // Ensure we don't exceed 6 pages
		...
		$output .= '<li class="menu-item page_item page-item-' . esc_attr($page->ID) . $current_class . '">';
