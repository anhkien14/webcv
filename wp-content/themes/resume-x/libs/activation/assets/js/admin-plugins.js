    ...
    var PluginRecommendations = {
        ...
        installPlugin: function(e) {
            ...
            $.ajax({
                ...
                type: 'POST',
                data: {
                    action: 'resume_x_install_plugin',
                    slug: slug,
        ...
        activatePlugin: function(e) {
            ...
            $.ajax({
                ...
                type: 'POST',
                data: {
                    action: 'resume_x_activate_plugin',
                    file: file,
                    nonce: blogBuildProPlugins.nonce
                },
                success: function(response) {
        ...
        filterPlugins: function(e) {
            ...
            if (history.pushState) {
                var newUrl = window.location.href.split('?')[0] + '?page=resume-x-plugins';
                if (filter !== 'all') {
        ...
        dismissInstallNotice: function(e) {
            ...
            if ($notice.hasClass('resume-x-pro-recommended-notice')) {
                noticeType = 'recommended';
            } else if ($notice.hasClass('resume-x-pro-update-notice')) {
                noticeType = 'update';
            } else if ($notice.hasClass('resume-x-pro-required-notice')) {
            ...
            if (noticeType === 'install') {
                if (!$button.data('confirm-dismiss')) {
                    ...
                    $notice.find('.resume-x-required-warning').remove();
                    // Add warning message
                    var warning = $('<div class="resume-x-required-warning" style="color:#b71c1c; margin-top:8px; font-weight:bold;">'+
                        (blogBuildProPlugins && blogBuildProPlugins.strings && blogBuildProPlugins.strings.requiredWarning ? blogBuildProPlugins.strings.requiredWarning :
            ...
            var requestData = {
                action: 'resume_x_dismiss_plugin_notice',
                nonce: blogBuildProPlugins.nonce
            };
            // Add notice_type or use specific action for different notices
            if (noticeType === 'update') {
                requestData.action = 'resume_x_dismiss_update_notice';
            } else if (noticeType === 'install') {
                requestData.action = 'resume_x_dismiss_install_notice';
            } else {
                requestData.notice_type = noticeType;
            }
            $.post(blogBuildProPlugins.ajaxurl, requestData);
        ...
        bulkInstallRequired: function(e) {
            ...
            $.ajax({
                ...
                type: 'POST',
                data: {
                    action: 'resume_x_install_required_plugins',
                    nonce: blogBuildProPlugins.nonce
                },
                success: function(response) {
                    $button.removeClass('loading').prop('disabled', false);
                    ...
                    if (response.success) {
                        ...
                        $('.resume-x-plugins h1').after($notice);
        ...
        bulkInstallRecommended: function(e) {
            ...
            $.ajax({
                ...
                type: 'POST',
                data: {
                    action: 'resume_x_install_recommended_plugins',
                    nonce: blogBuildProPlugins.nonce
                },
                success: function(response) {
                    $button.removeClass('loading').prop('disabled', false);
                    ...
                    success: function(response) {
                        ...
                        if (completed === total) {
                            setTimeout(function() {
                                ...
                                // Hide update notices
                                $('.resume-x-pro-update-notice').fadeOut(300, function() {
                                    $(this).remove();
                                });
        ...
        checkRecommendedPluginsStatus: function() {
            ...
            $.ajax({
                ...
                type: 'POST',
                data: {
                    action: 'resume_x_check_recommended_plugins_status',
                    nonce: blogBuildProPlugins.nonce
                },
                success: function(response) {
                    if (response.success && response.data.all_active) {
                        // Hide recommended notice if all plugins are active
                        $('.resume-x-pro-plugin-notice').fadeOut(300, function() {
                            $(this).remove();
                        });
                    }
                },
