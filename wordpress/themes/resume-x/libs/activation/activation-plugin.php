 ...
 */
if (!class_exists('resume_x_Pro_Silent_Upgrader_Skin')) {
    
    /**
     * Load the Silent Upgrader Skin class when needed
     */
    function resume_x_load_silent_upgrader_skin() {
        if (!class_exists('WP_Upgrader_Skin')) {
            require_once ABSPATH . 'wp-admin/includes/class-wp-upgrader.php';
        }
        
        if (!class_exists('resume_x_Pro_Silent_Upgrader_Skin')) {
            class resume_x_Pro_Silent_Upgrader_Skin extends WP_Upgrader_Skin {
 ...
 */
class resume_x_Pro_Plugin_Recommendations {
    ...
    public function __construct() {
        $this->plugin_dir = get_template_directory() . '/libs/activation';
        $this->plugin_url = get_template_directory_uri() . '/libs/activation';
        ...
        add_action('wp_ajax_resume_x_install_plugin', array($this, 'ajax_install_plugin'));
        add_action('wp_ajax_resume_x_activate_plugin', array($this, 'ajax_activate_plugin'));
        add_action('wp_ajax_resume_x_update_plugin', array($this, 'ajax_update_plugin'));
        add_action('wp_ajax_resume_x_dismiss_plugin_notice', array($this, 'ajax_dismiss_plugin_notice'));
        add_action('wp_ajax_resume_x_dismiss_update_notice', array($this, 'ajax_dismiss_update_notice'));
        add_action('wp_ajax_resume_x_dismiss_install_notice', array($this, 'ajax_dismiss_install_notice'));
        add_action('wp_ajax_resume_x_update_all_plugins', array($this, 'ajax_update_all_plugins'));
        add_action('wp_ajax_resume_x_install_required_plugins', array($this, 'ajax_install_required_plugins'));
        add_action('wp_ajax_resume_x_install_recommended_plugins', array($this, 'ajax_install_recommended_plugins'));
        add_action('wp_ajax_resume_x_check_recommended_plugins_status', array($this, 'ajax_check_recommended_plugins_status'));
        add_action('wp_ajax_resume_x_check_recommended_plugins_status', array($this, 'ajax_check_recommended_plugins_status'));
    ...
    public function add_admin_menu() {
        add_theme_page(
            __('Recommended Plugins', 'resume-x'),
            __('Recommended Plugins', 'resume-x'),
            'manage_options',
            'resume-x-plugins',
    ...
    public function get_recommended_plugins() {
        ...
        $default_plugins = resume_x_get_default_plugin_data();
         ...
         */
        $plugins = apply_filters('resume_x_recommended_plugins', $default_plugins);
         ...
         */
        $plugins = apply_filters('resume_x_add_recommended_plugins', $plugins);
         ...
         * @param array $plugins Current plugin configuration array
         */
        $plugins = apply_filters('resume_x_filter_plugins_by_category', $plugins);
    ...
    public function render_admin_page() {
        ...
        ?>
        <div class="wrap resume-x-plugins">
            <h1><?php printf(esc_html__('Recommended Plugins for %s', 'resume-x'), $this->theme_name); ?></h1>
            <p class="description">
                <?php printf(esc_html__('These plugins are recommended to enhance your %s theme experience. They work perfectly with the theme\'s features and animations.', 'resume-x'), $this->theme_name); ?>
            </p>
            ...
            <div class="notice notice-warning">
                <p>
                    <strong><?php esc_html_e('Missing Required Plugins:', 'resume-x'); ?></strong>
                    <?php printf(esc_html__('%d required plugins are missing or inactive.', 'resume-x'), count($missing_required)); ?>
                    <button id="install-required-plugins" class="button button-primary" style="margin-left: 10px;">
                        <?php esc_html_e('Install Required Plugins', 'resume-x'); ?>
                    </button>
                </p>
            </div>
            <?php endif; ?>
            ...
            <div class="plugin-filter-tabs">
                <?php if ($plugin_counts['all'] > 0) : ?>
                    <button class="filter-tab <?php echo ($first_active_tab === 'all') ? 'active' : ''; ?>" data-filter="all"><?php printf(esc_html__('All Plugins (%d)', 'resume-x'), $plugin_counts['all']); ?></button>
                <?php endif; ?>
                
                <?php if ($plugin_counts['featured'] > 0) : ?>
                    <button class="filter-tab <?php echo ($first_active_tab === 'featured') ? 'active' : ''; ?>" data-filter="featured"><?php printf(esc_html__('Featured (%d)', 'resume-x'), $plugin_counts['featured']); ?></button>
                <?php endif; ?>
                
                <?php if ($plugin_counts['required'] > 0) : ?>
                    <button class="filter-tab <?php echo ($first_active_tab === 'required') ? 'active' : ''; ?>" data-filter="required"><?php printf(esc_html__('Required (%d)', 'resume-x'), $plugin_counts['required']); ?></button>
                <?php endif; ?>
                
                <?php if ($plugin_counts['active'] > 0) : ?>
                    <button class="filter-tab <?php echo ($first_active_tab === 'active') ? 'active' : ''; ?>" data-filter="active"><?php printf(esc_html__('Active (%d)', 'resume-x'), $plugin_counts['active']); ?></button>
                <?php endif; ?>
                
                <?php if ($plugin_counts['inactive'] > 0) : ?>
                    <button class="filter-tab <?php echo ($first_active_tab === 'inactive') ? 'active' : ''; ?>" data-filter="inactive"><?php printf(esc_html__('Inactive (%d)', 'resume-x'), $plugin_counts['inactive']); ?></button>
                <?php endif; ?>
                
                <?php if ($plugin_counts['not-installed'] > 0) : ?>
                    <button class="filter-tab <?php echo ($first_active_tab === 'not-installed') ? 'active' : ''; ?>" data-filter="not-installed"><?php printf(esc_html__('Not Installed (%d)', 'resume-x'), $plugin_counts['not-installed']); ?></button>
                <?php endif; ?>
            </div>
                ...
                ?>
                    <div class="plugin-card <?php echo esc_attr($featured_class); ?>" data-status="<?php echo esc_attr($status); ?>" data-featured="<?php echo $plugin['featured'] ? '1' : '0'; ?>" data-required="<?php echo $plugin['required'] ? '1' : '0'; ?>" data-local="<?php echo $is_local ? '1' : '0'; ?>" data-slug="<?php echo esc_attr($slug); ?>">
                        <?php if ($plugin['featured']) : ?>
                            <div class="featured-badge"><?php esc_html_e('Featured', 'resume-x'); ?></div>
                        <?php endif; ?>
                        
                        <?php if ($plugin['required']) : ?>
                            <div class="required-badge"><?php esc_html_e('Required', 'resume-x'); ?></div>
                        <?php endif; ?>
                        
                        <?php if ($is_local) : ?>
                            <div class="local-plugin-badge"><?php esc_html_e('Local', 'resume-x'); ?></div>
                        <?php endif; ?>
    ...
    private function render_plugin_button($slug, $plugin, $status) {
        ...
        switch ($status) {
            case 'active':
                echo '<span class="button disabled">' . esc_html__('Active', 'resume-x') . '</span>';
                break;
            case 'needs-update':
                echo '<button class="button button-secondary update-plugin" data-slug="' . esc_attr($slug) . '" data-file="' . esc_attr($plugin['file']) . '" data-is-local="' . ($is_local ? '1' : '0') . '">' . esc_html__('Update', 'resume-x') . '</button>';
                break;
            case 'inactive':
                echo '<button class="button button-primary activate-plugin" data-slug="' . esc_attr($slug) . '" data-file="' . esc_attr($plugin['file']) . '">' . esc_html__('Activate', 'resume-x') . '</button>';
                break;
            case 'inactive-needs-update':
                echo '<button class="button button-secondary update-plugin" data-slug="' . esc_attr($slug) . '" data-file="' . esc_attr($plugin['file']) . '" data-is-local="' . ($is_local ? '1' : '0') . '">' . esc_html__('Update', 'resume-x') . '</button>';
                break;
            case 'not-installed':
                if ($is_local) {
                    echo '<button class="button button-primary install-plugin" data-slug="' . esc_attr($slug) . '" data-is-local="1">' . esc_html__('Install', 'resume-x') . '</button>';
                } else {
                    echo '<button class="button button-primary install-plugin" data-slug="' . esc_attr($plugin['slug']) . '" data-is-local="0">' . esc_html__('Install', 'resume-x') . '</button>';
                }
                break;
        }
        ...
        if ($current_version && $required_version) {
            ...
            printf(esc_html__('Current: %s | Required: %s', 'resume-x'), $current_version, $required_version);
            echo '</small>';
        } elseif ($current_version) {
            echo '<small class="version-info">';
            printf(esc_html__('Version: %s', 'resume-x'), $current_version);
            echo '</small>';
        } elseif ($required_version) {
            echo '<small class="version-info">';
            printf(esc_html__('Required Version: %s', 'resume-x'), $required_version);
            echo '</small>';
        }
    }
    ...
    private function get_status_text($status) {
        switch ($status) {
            case 'active':
                return __('Active', 'resume-x');
            case 'needs-update':
                return __('Update Available', 'resume-x');
            case 'inactive':
                return __('Installed but not active', 'resume-x');
            case 'inactive-needs-update':
                return __('Update Available (Inactive)', 'resume-x');
            case 'not-installed':
                return __('Not installed', 'resume-x');
            default:
                return __('Unknown', 'resume-x');
        }
    }
    ...
    public function show_update_notice() {
        ...
        if (!in_array($screen->id, array('dashboard', 'themes', 'plugins', 'appearance_page_resume-x-plugins'))) {
            return;
        }
        
        // Check if user dismissed the update notice
        $dismissed_time = get_user_meta(get_current_user_id(), 'resume_x_hide_update_notice_time', true);
        ...
        ?>
        <div class="notice notice-warning is-dismissible resume-x-pro-update-notice">
            <div class="plugin-update-content">
                <div style="flex: 1;">
                    <h3><?php esc_html_e('Plugin Updates Available', 'resume-x'); ?></h3>
                    <p><?php printf(esc_html__('%d plugins have updates available. Keep your plugins up to date for better security and performance.', 'resume-x'), count($plugins_need_update)); ?></p>
                    <div style="display: flex; gap: 12px; align-items: center; flex-wrap: wrap; margin-top: 15px;">
                        <button id="update-all-plugins" class="button button-primary"><?php esc_html_e('Update All Plugins', 'resume-x'); ?></button>
                        <a href="<?php echo esc_url(admin_url('themes.php?page=resume-x-plugins')); ?>" class="button button-secondary"><?php esc_html_e('Manage Plugins', 'resume-x'); ?></a>
                    </div>
                </div>
    ...
    public function ajax_dismiss_update_notice() {
        check_ajax_referer('resume_x_plugins_nonce', 'nonce');
        
        // Save the current timestamp when update notice is dismissed
        update_user_meta(get_current_user_id(), 'resume_x_hide_update_notice_time', time());
    ...
    public function ajax_dismiss_install_notice() {
        ...
        update_user_meta(get_current_user_id(), 'resume_x_hide_install_notice_time', time());
        wp_send_json_success();
    }
    ...
    public function ajax_dismiss_plugin_notice() {
        ...
        if ($notice_type === 'recommended') {
            ...
            update_user_meta(get_current_user_id(), 'resume_x_info_recomend_plugins', time());
        }
    ...
    public function ajax_update_all_plugins() {
        ...
        if (!current_user_can('update_plugins')) {
            wp_die(__('You do not have permission to update plugins.', 'resume-x'));
        }
        ...
        if ($success_count === $total_count) {
            wp_send_json_success(array(
                'message' => sprintf(__('%d plugins updated successfully.', 'resume-x'), $success_count),
                'results' => $results
            ));
        } else {
            wp_send_json_error(array(
                'message' => sprintf(__('%d of %d plugins updated successfully.', 'resume-x'), $success_count, $total_count),
                'results' => $results
            ));
        }
    }
    ...
    public function enqueue_admin_scripts($hook) {
        ...
        if (!in_array($hook, array('appearance_page_resume-x-plugins', 'index.php', 'themes.php', 'plugins.php'))) {
            return;
        }
        
        wp_enqueue_script(
            'resume-x-plugins-admin',
        ...
        wp_localize_script('resume-x-plugins-admin', 'blogBuildProPlugins', array(
            'ajaxurl' => admin_url('admin-ajax.php'),
            'nonce' => wp_create_nonce('resume_x_plugins_nonce'),
            'strings' => array(
                'installing' => __('Installing...', 'resume-x'),
                'activating' => __('Activating...', 'resume-x'),
                'updating' => __('Updating...', 'resume-x'),
                'installed' => __('Installed', 'resume-x'),
                'activated' => __('Activated', 'resume-x'),
                'updated' => __('Updated', 'resume-x'),
                'active' => __('Active', 'resume-x'),
                'inactive' => __('Inactive', 'resume-x'),
                'notInstalled' => __('Not Installed', 'resume-x'),
                'needsUpdate' => __('Update Available', 'resume-x'),
                'inactiveNeedsUpdate' => __('Update Available (Inactive)', 'resume-x'),
                'install' => __('Install', 'resume-x'),
                'activate' => __('Activate', 'resume-x'),
                'update' => __('Update', 'resume-x'),
                'error' => __('Error occurred', 'resume-x'),
                'requiredWarning' => __('You must install and activate all required plugins before dismissing this notice. Click Dismiss again to hide this warning.', 'resume-x'),
            )
        ));
    ...
    public function ajax_install_plugin() {
        ...
        if (!current_user_can('install_plugins')) {
            wp_die(__('You do not have permission to install plugins.', 'resume-x'));
        }
        ...
        if ($is_local) {
            ...
            if (!isset($plugins[$slug])) {
                wp_send_json_error(__('Plugin not found in recommendations.', 'resume-x'));
            }
            ...
            if (!isset($plugin['source']) || !file_exists($plugin['source'])) {
                ...
                if (!isset($plugin['source']) || !file_exists($plugin['source'])) {
                    wp_send_json_error(__('Local plugin zip file not found. Please ensure the plugin zip file exists in the theme directory.', 'resume-x'));
                }
            }
            ...
            if (!is_writable(WP_PLUGIN_DIR)) {
                wp_send_json_error(__('Plugins directory is not writable. Please check directory permissions.', 'resume-x'));
            }
            
            $result = $this->install_local_plugin($plugin);
            
            if ($result) {
                wp_send_json_success(__('Local plugin installed successfully.', 'resume-x'));
            } else {
                wp_send_json_error(__('Local plugin installation failed. Please check error logs for details.', 'resume-x'));
            }
        } else {
            ...
            if (is_wp_error($api)) {
                wp_send_json_error(__('Plugin information could not be retrieved.', 'resume-x'));
            }
            ...
            if ($install) {
                wp_send_json_success(__('Plugin installed successfully.', 'resume-x'));
            } else {
                wp_send_json_error(__('Plugin installation failed.', 'resume-x'));
            }
        }
    }
    ...
    public function ajax_activate_plugin() {
        ...
        if (!current_user_can('activate_plugins')) {
            wp_die(__('You do not have permission to activate plugins.', 'resume-x'));
        }
        ...
        if (is_wp_error($result)) {
            ...
            wp_send_json_success(__('Plugin activated successfully.', 'resume-x'));
        }
    }
    ...
    public function ajax_update_plugin() {
        ...
        if (!isset($plugins[$slug])) {
            wp_send_json_error(__('Plugin not found.', 'resume-x'));
        }
        ...
        if ($is_local) {
            ...
            if ($result) {
                wp_send_json_success(__('Local plugin updated successfully.', 'resume-x'));
            } else {
                wp_send_json_error(__('Failed to update local plugin.', 'resume-x'));
            }
        } else {
            ...
            if ($result) {
                wp_send_json_success(__('Plugin updated successfully.', 'resume-x'));
            } else {
                wp_send_json_error(__('Failed to update plugin.', 'resume-x'));
            }
        }
    }
    ...
    public function ajax_install_required_plugins() {
        ...
        if ($success_count === $total_count) {
            wp_send_json_success(array(
                'message' => sprintf(__('%d required plugins installed and activated successfully.', 'resume-x'), $success_count),
                'results' => $results
            ));
        } else {
            wp_send_json_error(array(
                'message' => sprintf(__('%d of %d required plugins installed successfully.', 'resume-x'), $success_count, $total_count),
                'results' => $results
            ));
        }
    }
    ...
    public function ajax_install_recommended_plugins() {
        ...
        if ($success_count === $total_count) {
            wp_send_json_success(array(
                'message' => sprintf(__('%d recommended plugins installed and activated successfully.', 'resume-x'), $success_count),
                'results' => $results
            ));
        } else {
            wp_send_json_error(array(
                'message' => sprintf(__('%d of %d recommended plugins installed successfully.', 'resume-x'), $success_count, $total_count),
                'results' => $results
            ));
        }
    }
    ...
    public function ajax_check_recommended_plugins_status() {
        ...
        wp_send_json_success(array(
            ...
            'message' => $all_active ? __('All recommended plugins are active!', 'resume-x') : __('Some recommended plugins are not active.', 'resume-x')
        ));
    }
    ...
    public function show_recommendation_notice() {
        ...
        if (!empty($missing_required)) {
            ...
            $dismissed_time = get_user_meta(get_current_user_id(), 'resume_x_hide_install_notice_time', true);
            $show_required_notice = !$dismissed_time || (time() - $dismissed_time) >= (7 * 24 * 60 * 60); // 7 days
            
            if ($show_required_notice) {
                ?>
            <div class="notice notice-warning resume-x-pro-required-notice">
                <p>
                    <strong><?php echo esc_html($this->theme_name); ?></strong> - 
                    <?php printf(esc_html__('%d required plugins are missing. Please install them for optimal theme functionality.', 'resume-x'), count($missing_required)); ?>
                    <br><br>
                    <button id="install-required-plugins" class="button button-primary" style="margin-right: 10px;">
                        <?php esc_html_e('Install Required Plugins', 'resume-x'); ?>
                    </button>
                    <a href="<?php echo esc_url(admin_url('themes.php?page=resume-x-plugins')); ?>" class="button button-secondary">
                        <?php esc_html_e('View All Plugins', 'resume-x'); ?>
                    </a>
                     <button id="install-dismiss" class="button install-dismiss" style="margin-left: 10px;">
                        <?php esc_html_e('Dismiss', 'resume-x'); ?>
                    </button>
    ...
    private function show_recommended_plugins_notice() {
        ...
        $dismissed_time = get_user_meta(get_current_user_id(), 'resume_x_info_recomend_plugins', true);
        ...
        ?>
        <div class="notice notice-info is-dismissible resume-x-pro-recommended-notice">
            <div class="plugin-recommended-content">
                <div style="flex: 1;">
                    <h3><?php printf(esc_html__('Recommended Plugins for %s', 'resume-x'), $this->theme_name); ?></h3>
                    <p><?php printf(esc_html__('We recommend %d additional plugins to enhance your %s theme experience.', 'resume-x'), count($inactive_recommended), $this->theme_name); ?></p>
                    <div style="display: flex; gap: 12px; align-items: center; flex-wrap: wrap; margin-top: 15px;">
                        <button id="install-recommended-plugins" class="button button-primary"><?php esc_html_e('Install Recommended Plugins', 'resume-x'); ?></button>
                        <a href="<?php echo esc_url(admin_url('themes.php?page=resume-x-plugins')); ?>" class="button button-secondary"><?php esc_html_e('View All Plugins', 'resume-x'); ?></a>
    ...
    public function auto_install_required_plugins() {
        ...
        set_transient('resume_x_theme_activated', true, 30);
        ...
        set_transient('resume_x_plugins_installed', true, 300);
    }
    ...
    public function check_required_plugins() {
        ...
        if (get_transient('resume_x_plugins_checked')) {
            return;
        }
        
        // Set transient to avoid repeated checks
        set_transient('resume_x_plugins_checked', true, 3600); // 1 hour
    ...
    private function install_plugin_silent($plugin) {
        try {
            ...
            resume_x_load_silent_upgrader_skin();
    ...
    private function install_wordpress_org_plugin($plugin) {
        try {
            ...
            $upgrader = new Plugin_Upgrader(new resume_x_Pro_Silent_Upgrader_Skin());
            $install = $upgrader->install($api->download_link);
    ...
    private function migrate_notice_dismissal() {
        ...
        if (get_option('resume_x_notice_migration_done')) {
            return;
        }
        
        // Reset old dismissals for migration
        delete_user_meta(get_current_user_id(), 'resume_x_hide_required_plugins_notice');
        delete_user_meta(get_current_user_id(), 'resume_x_hide_recommended_plugins_notice');
        delete_user_meta(get_current_user_id(), 'resume_x_hide_update_notice_time');
        
        // Mark migration as done
        update_option('resume_x_notice_migration_done', true);
    }
...
new resume_x_Pro_Plugin_Recommendations();
