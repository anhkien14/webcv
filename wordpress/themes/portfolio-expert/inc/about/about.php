			...
			'recommended_actions'        => array(
				...
				'content'            => array(
					'magical-blocks' => array(
						'title'       => __('WPEPP – Essential Security, Password Protect & Login Page Customizer', 'portfolio-expert'),
						'description' => __('WPEPP best for Best Essential security, password protected content, block AI crawlers, limit login attempts, CPU monitor & login customizer with live preview. Total site protection! WPEPP is the all-in-one WordPress essential security and login page customizer plugin.', 'portfolio-expert'),
						'plugin_slug' => 'wp-edit-password-protected',
						'id' => 'wp-edit-password-protected'
					),
					'go-pro' => array(
...
{
	$hide_date = get_option('portfolio_expert_info_text2');
	if (!empty($hide_date)) {
...
function portfolio_expert_new_optins_texts_init()
{
	...
	if (isset($_GET['xbnotice']) && $_GET['xbnotice'] == 1) {
		update_option('portfolio_expert_info_text2', current_time('mysql'));
	}
}
